import { Claim, ClaimType, ClaimCategory, ClaimStatus, SubmitClaimRequest, Source, Evidence } from '../models/types';
import { CryptoService } from './crypto.service';
import { DatabaseService } from './database.service';
import { generateCanonicalHash } from '../utils/helpers';
...
export class ClaimService {
   ...
   */
  async submitClaim(request: SubmitClaimRequest, submitterWalletId: string): Promise<Claim> {
    // Validate claim submission
    this.validateClaimRequest(request);

    // Check for duplicate or similar existing claims
    const canonicalHash = generateCanonicalHash(request);
   ...
   */
  async enterCommitPhase(claimId: string): Promise<void> {
    await this.updateClaimStatus(claimId, ClaimStatus.COMMIT_PHASE);
  }

  /**
   * Enter reveal phase for a claim
   */
  async enterRevealPhase(claimId: string): Promise<void> {
    await this.updateClaimStatus(claimId, ClaimStatus.REVEAL_PHASE);
  }

  /**
   * Find claims that are approaching deadline
   */
  async findClaimsNearDeadline(hours: number): Promise<Claim[]> {
   ...
   */
  private validateClaimRequest(request: SubmitClaimRequest): void {
    if (!request.subject || !request.predicate || !request.object) {
      throw new Error('Claim must have subject, predicate, and object');
    }

    if (!request.type || !Object.values(ClaimType).includes(request.type)) {
      throw new Error('Invalid claim type');
    }

    if (!request.category || !Object.values(ClaimCategory).includes(request.category)) {
   ...
   */
  private async findSimilarClaim(canonicalHash: string, request: SubmitClaimRequest): Promise<Claim | null> {
   ...
   */
  async getAvailableClaimsForVerification(filters: ClaimFilters): Promise<Claim[]> {
