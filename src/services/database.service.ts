...
export class DatabaseService {
  ...
  constructor(dbPath?: string) {
    const path = dbPath || join(process.cwd(), 'data', 'truthmarket.db');
    ensureDirectoryExists(path);
    this.db = open({ filename: path, eager: true });
  ...
  async saveClaim(claim: Claim): Promise<void> {
    ...
    stmt.run(
      claim.id,
      claim.submitterWalletId,
      claim.type,
      claim.subject,
      claim.predicate,
      claim.object,
      claim.canonicalHash,
      claim.timeWindow?.start?.toISOString(),
      claim.timeWindow?.end?.toISOString(),
      claim.timeWindow?.asOf?.toISOString(),
      claim.jurisdiction,
      claim.category,
      claim.bounty,
      claim.stake,
      claim.deadline.toISOString(),
      claim.status,
  ...
  searchClaims(filters: {
    ...
    if (!filters.includeResolved) {
      query += ' AND status != ?';
      params.push(ClaimStatus.FINALIZED);
    }
  ...
  findClaimsNearDeadline(hours: number): Claim[] {
    ...
    const rows = stmt.all(ClaimStatus.FINALIZED);
    return rows.map(row => this.mapRowToClaim(row));
  }
  ...
  saveVerification(verification: Verification): Promise<void> {
    const stmt = this.db.prepare(`
      INSERT INTO verifications (
        id, claim_id, verifier_wallet_id, commit_hash, verdict, confidence, evidence, stake, revealed_at, timestamp
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    stmt.run(
      ...
      verification.verifierWalletId,
      verification.commitHash,
      verification.verdict,
      verification.confidence,
      JSON.stringify(verification.evidence),
      verification.stake,
      verification.revealedAt?.toISOString(),
      verification.timestamp.toISOString()
    );
  ...
  saveConsensusResult(result: ConsensusResult): void {
    ...
    stmt.run(
      ...
      result.confidence,
      result.isFinal,
      result.finalizedAt.toISOString(),
      JSON.stringify(result.participatingVerifiers),
  ...
  private mapRowToVerification(row: any): Verification {
  ...
  private mapRowToConsensusResult(row: any): ConsensusResult {
    return {
      claimId: row.claim_id,
      verdict: row.verdict as Verdict,
      confidence: row.confidence,
