...
import { SubmitVerificationRequest } from '../models/types';
...
export class VerificationAPI {
  ...
  private extractAuthFromRequest(req: Request): { walletId: string; signature: string } {
    ...
    if (!walletId || !signature) {
      throw new Error('Missing authentication: walletId and signature required');
    }

    return { walletId, signature };
  }
   ...
   */
  async submitCommit(req: Request, res: Response): Promise<void> {
    try {
      ...
      const escrowTransactionId = await this.walletService.createCommitTransaction(
        walletId,
        1, // Minimum stake of 1 satoshi
        claimId,
        commit.commitHash
      );
   ...
   */
  async revealVerification(req: Request, res: Response): Promise<void> {
    try {
      // Authenticate request
      const { walletId, signature } = this.extractAuthFromRequest(req);
      const isValid = await this.walletService.validateSignature(
      ...
      const reveal = await this.verificationService.revealVerification(
        claimId,
        walletId,
        { claimId, verdict, confidence, evidence },
        nonce,
        signature
      );
   ...
   */
  async getConsensus(req: Request, res: Response): Promise<void> {
    try {
      const { claimId } = req.params;
