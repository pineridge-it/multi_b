 ...
 */
export const generateCanonicalHash = (request: SubmitClaimRequest): string => {
 ...
 */
export function formatClaimForDisplay(request: SubmitClaimRequest): string {
  let formatted = '';
...
function getZValue(confidence: number): number {
  ...
  if (confidence >= 0.70) return 1.036;
  return 1.0; // Default
}