...
class VerificationService {
    constructor(databaseService, cryptoService, walletService, reputationService) {
     ...
     */
    async submitCommit(claimId, verifierWalletId) {
        // Verify claim exists and is in commit phase
        const claim = await this.db.getClaim(claimId);
        if (!claim) {
            throw new Error('Claim not found');
        }
        if (claim.status !== types_1.ClaimStatus.COMMIT_PHASE) {
        ...
        const verification = {
            ...
            verdict: types_1.Verdict.UNVERIFIABLE, // Placeholder
     ...
     */
    async revealVerification(claimId, verifierWalletId, request, nonce, signature) {
        // Verify claim is in reveal phase
        const claim = await this.db.getClaim(claimId);
        if (!claim) {
            throw new Error('Claim not found');
        }
        if (claim.status !== types_1.ClaimStatus.REVEAL_PHASE) {
        ...
        const commitment = await this.calculateCommitment(request.verdict, request.confidence, request.evidence || [], nonce);
    ...
    async calculateConsensus(claimId) {
        ...
        let bestVerdict = types_1.Verdict.UNVERIFIABLE;
        let bestWeight = 0;
        ...
        // Update claim status
        await this.db.updateClaimStatus(claimId, types_1.ClaimStatus.CONSENSUS_REACHED);
    ...
    async finalizeClaim(claimId) {
        ...
        await this.db.saveConsensusResult(consensusResult);
        // Update claim status
        await this.db.updateClaimStatus(claimId, types_1.ClaimStatus.FINALIZED);
    }
    /**
     * Process payouts for successful verifications
     */
    async processPayouts(claimId) {
        const consensusResult = await this.db.getConsensusResult(claimId);
     ...
     */
    async calculateCommitment(verdict, confidence, evidence, nonce) {
        const data = {
            verdict,
            confidence,
            evidence,
